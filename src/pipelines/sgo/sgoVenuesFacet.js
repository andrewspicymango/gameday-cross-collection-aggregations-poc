 ...
 * SGO Venues Facet
 *
 * Aggregation facet pipeline for resolving venue references for a given SGO.
 *
 ...
 *   2. Lookup venue documents (collection: "venues") that have this SGO
 *      in their sgoMemberships array by matching the derived SGO key.
 *   3. From each matched venue, create venue pairs with id, scope, and composite key.
 *      Only venues with valid _externalId and _externalIdScope are included.
 *   4. Project a final object containing:
 *        - ids: unique set of matched venues._id (typically ObjectId)
 *        - keys: unique set of external key strings ("<id><keySeparator><scope>") for the venues
 *
 ...
 *   - Only venues with string-typed, non-empty _externalId and _externalIdScope are considered.
 ...
 *     the set of venue references where the SGO has membership.
 *
 * @constant
 * @type {Array<Object>}
 * @name sgoVenuesFacet
 */
const sgoVenuesFacet = [
	// Build the SGO key for matching
	{ $addFields: { sgoKey: { $concat: ['$_externalId', keySeparator, '$_externalIdScope'] } } },

	// Lookup venues where this SGO has membership
	{
		$lookup: {
			from: 'venues',
			let: { sgoKey: '$sgoKey' },
			pipeline: [
				...
				// Filter out venues with incomplete keys
				{ $match: { venuePair: { $ne: null } } },
			],
			as: 'venueDocs',
		},
	},

	// Final projection with unique venue IDs and keys
	{
		$project: {
			_id: 0,
			ids: { $setUnion: [{ $map: { input: '$venueDocs', as: 'v', in: '$$v._id' } }, []] },
			keys: { $setUnion: [{ $map: { input: '$venueDocs', as: 'v', in: '$$v.venuePair.key' } }, []] },
		},
	},
];

////////////////////////////////////////////////////////////////////////////////
exports.sgoVenuesFacet = sgoVenuesFacet;
